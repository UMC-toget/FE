/**
 * E03 편지지 색상 8종 (피그마 '편지지 입력전/접힘/열림' 컴포넌트셋 #1714:70785 실측값)
 *
 * 사용자가 고르는 선택지(도메인 데이터)이자 런타임 동적 값이라 Tailwind 클래스 대신
 * style prop으로 적용 — 색상값은 이 파일 한 곳에서만 관리.
 * 기존 @theme 토큰과 겹치는 값은 CSS 변수를 참조하고, 편지지 전용 색만 리터럴 사용.
 * placeholder는 테두리 강조색 솔리드 (그린 #7EB900, 화이트 #ACA6AB는 피그마 실측)
 */
