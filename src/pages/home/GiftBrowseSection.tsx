...
export default function GiftBrowseSection() {
  ...
  // 비로그인 상태에서 상품 카드를 선택하면 로그인 화면으로 보냅니다 (피그마 B01 기준).
  // TODO: 상품 상세 화면이 생기면 로그인 상태일 때 해당 화면으로 라우팅
      ...
      {filteredProducts.length > 0 ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-4">
          ...
          {filteredProducts.map((product, index) => (
            <ProductCard key={product.id} product={product} rank={index + 1} onClick={handleProductClick} />
          ))}
        </div>
      ) : (
